...
export function Tab({ componentsForTabs }){
    ...
    return(
        <>
            ...
            { componentsForTabs.map((val, idx) => {
                return (
                    <TabSelector activeTab={idxSelected} selectedIdx={(e) => {console.log(e); setIdxSelected(e)}} idx={idx} title={val.name} icon={val.icon ? val.icon : null} />
                )
            }) }
            </View>
        </>
...
function TabSelector({ title, icon, selectedIdx, activeTab, idx }){
    return(
        <TouchableOpacity 
